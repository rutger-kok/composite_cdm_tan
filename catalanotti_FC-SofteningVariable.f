...
c        A6 = 2.0d0*initialcharLength*SL*SL
c     1       /(2.0d0*g12*G6-initialcharLength*SL*SL)
...
c 100     FORMAT (E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,A,E,
c      1          A,E,A,E,A,E,A,E)
c         WRITE(1,100) trialStress(1), ',',
c      1  trialStress(2), ',', trialStress(3), ',',
c      2  trialStress(4), ',', trialStress(5), ',',
c      3  trialStress(6), ',', trialStrain(1), ',',
c      4  trialStrain(2), ',', trialStrain(3), ',',
c      5  trialStrain(4), ',', trialStrain(5), ',',
c      6  trialStrain(6), ',', xOmega1, ',', xOmega2, ',', xOmega3, ',',
c      7  xOmega4, ',', xOmega5, ',', xOmega6
...
c          WRITE(1,*) 'fx', fx
          ...
          ! update x:
          gamma = gamma - d
c          WRITE(1,12) k,gamma
c 12        FORMAT('After', i3, ' iterations, x = ', e22.15)
        ENDDO
      ...
      FUNCTION FUNC(gamma1, trialStress, X)
        ...
        ! Eq. 88 CLN
c        WRITE(1,*) 'gamma1', gamma1
c        WRITE(1,*) 'trialStress1', trialStress
     ...
     2         - abs(trialStress(4))*cos(2.0d0*gamma1) 
      ...
      FUNCTION DFUNC(gamma2, trialStress, X)
        ...
        ! Eq. 89 CLN
c        WRITE(1,*) 'gamma2', gamma2
c        WRITE(1,*) 'trialStress2', trialStress
...
* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> *
     ...
     2                    - trialStressT(1)*v*u
        ...
        subroutine SofteningParam(lch,X,E,G,trialStrain,k,A)     !It will only be used when phi=1
        real*8 E,X,lch,G,trialStrain
        ...
        integer i, j, length_integral, n, k
           ...
           do i = 1,length_integral
              ...
              phi(i) = dsqrt((E*Strain(i)/X)**2.0d0)          !!New value of phi
           enddo
...
666        if (Error.gt.0.01)then
              ...
              n = n +1
c              if (n.gt.9999)then
c                      A = (2*lch*X*X/((2*E*G)-(lch*X*X)))
c                      goto 777
c              endif
c              if (A.lt.0.0d0)then
c                      A = (2*lch*X*X/((2*E*G)-(lch*X*X)))
c                      goto 777
c              endif
           else
