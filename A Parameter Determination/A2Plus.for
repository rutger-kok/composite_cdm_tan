...
200   FORMAT (A,A,A,A,A,A,A,A,A)
      WRITE(1,200) 'trialStress(2)', ',','trialStrain(2)', ',','FI_MT',
     1             ',', 'r2Plus', ',', 'd2Plus'  
...
C Initial approximation A1Minus
      ...
      A1 = 2.0d0*lch*YT*YT/(2.0d0*e22*G2plus-lch*YT*YT)
...
* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> *
      ...
      FUNCTION BRENT(func,A1,tol)
        ...
        x1 = A1/1000.0d0 !initial A variable, i.e. A0M from Maimi part II
        x2 = A1 ! A1M
...
* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> *
      FUNCTION INTG(A1)
        ...
        DO WHILE (abs(trialStress(2)).gt.fStress)
          call CDM(trialStrain,A1,trialStress)
          trialStrain(2) = trialStrain(2) + 0.00025
        END DO
        failStrain = trialStrain(2)

        ! iterate over strain values from 0 to failure strain
        n = 1000
        DO i = 0,n-1
          trialStrain(1) = 0.d0
          trialStrain(2) = (failStrain/n)*i
          trialStrain(3) = 0.d0
          trialStrain(4) = 0.d0
          trialStrain(5) = 0.d0
          trialStrain(6) = 0.d0
          tStrainArray(i+1) = trialStrain(2)
   
          call CDM(trialStrain,A1,trialStress)

          tStressArray(i+1) = trialStress(2)
        END DO 
...
C Trial stresses - pure transverse tension
...
C         trialStress(1) = C(1)*trialStrain(1)+C(4)*trialStrain(2)
C     1                   +C(5)*trialStrain(3)
...
C A parameters
        ...
        !A1Plus
        A(1) = (2.0d0*lch*XT*XT)/(2.0d0*e11*G1plus-lch*XT*XT)
        !A1Minus
...
100   FORMAT (E,A,E,A,E,A,E,A,E)
      WRITE(1,100) trialStress(2), ',',trialStrain(2), ',', FI_MT, ',',
     1             rNew3, ',', d2Plus
