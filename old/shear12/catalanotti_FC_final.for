C----------------------------------------------------------------------C
C     ABAQUS VUMAT USER SUBROUTINE: catalanotti_FC_altPhi.for          C
...
C Write only -
     *     stressNew, stateNew, enerInternNew, enerInelasNew )
     ...
     3     enerInternNew(nblock), enerInelasNew(nblock)
      ...
      REAL*8 trialStressP(6),delta_0,delta_c
...
C Loop through the gauss points
     ...
     1                    FI_MT,FI_MC)
        ...
        ! Determined outwith this script
        A1plus = 0.561992837659747
        A2plus = 0.129489324989586
        ...
        IF (rNew2.gt.1.0d0) THEN !Triangular damage dissipation
            ...
            IF (delta_c.lt.delta_0) THEN  !Needed for d.nq.0-0
                delta_c = 1.5*delta_0   !Adjustable for stability
            ENDIF
            d1Minus = (delta_c*(trialStrain(1)-delta_0))
     1                /(trialStrain(1)*(delta_c-delta_0))
            IF (d1Minus.lt.0.0d0) THEN
                d1Minus = 0.0d0
            ENDIF
        ...
        IF (rNew4.gt.1.0d0) THEN
            delta_0 = YC/e22
            delta_c = (2.0d0*G2minus/YC*1.0d0)
            IF (delta_c.lt.delta_0) THEN  !Needed for d.nq.0-0
                delta_c = 1.1*delta_0   !Adjustable for stability
            ENDIF
            d2Minus = (delta_c*(abs(trialStrain(2))-delta_0))
     1                /(abs(trialStrain(2))*(delta_c-delta_0))
            IF (d2Minus.lt.0.0d0) THEN
                d2Minus = 0.0d0
            ENDIF
            d2Minus = min(0.999,d2Minus)
        ELSE
            d2Minus = 0.0d0
        ENDIF
        ...
        IF (rNew3.gt.1.0d0) THEN !Triangular damage dissipation
            delta_0 = SL/e22
            delta_c = (2.0d0*G6/e22*1.0d0)
            IF (delta_c.lt.delta_0) THEN  !Needed for d.nq.0-0
                delta_c = 1.1*delta_0   !Adjustable for stability
            ENDIF
            d6 = (delta_c*(abs(trialStrain(4))-delta_0))
     1                /(abs(trialStrain(4))*(delta_c-delta_0))
            IF (d6.lt.0.0d0) THEN
                d6 = 0.0d0
            ENDIF
            d6 = min(0.999,d6)
